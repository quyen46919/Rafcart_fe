  ...
  <div class="col-span-3">
    ...
    <div class="px-4 py-3 w-[280px] h-[66px] shadow flex items-center gap-4">
      ...
      <div>
        ...
        <h4 class="text-gray-800 w-40 text-ellipsis overflow-hidden whitespace-nowrap text-lg capitalize font-medium">{{ props?.username }}</h4>
      </div>
    </div>
    <!-- account profile end -->
    ...
    <div class="mt-6 bg-white shadow rounded w-[280px] p-4 divide-y divide-gray-200 space-y-4 text-gray-600">
      <div v-for="menuItem in menuList" :key="menuItem.id" class="space-y-1 pl-8" :class="{ 'pt-4': menuItem.id !== 1 }">
        <router-link :to="menuItem.url" class="relative text-gray-800 font-medium capitalize hover:text-primary transition block" style="cursor: pointer;">
          {{ menuItem.label }}
        ...
        <div v-for="subItem in menuItem.subItems" :key="subItem.url">
          <router-link :to="subItem.url" class="hover:text-primary transition capitalize block" style="cursor: pointer;">{{ subItem.label }}</router-link>
        </div>
      </div>
    </div>
    <!-- profile links end -->
